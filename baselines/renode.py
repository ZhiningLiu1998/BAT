    ...
    def compute(self, pred, target):
        if self.loss_name == "ce":
            ...
            return F.cross_entropy(
                pred, target, weight=self.factor_train.to(self.device), reduction="none"
            )

        elif self.loss_name == "focal":
            labels_one_hot = (
                F.one_hot(target, self.cls_num).type(torch.FloatTensor).to(self.device)
            )
            ...
            temp_loss = F.binary_cross_entropy(
                input=pred, target=labels_one_hot, weight=weights, reduction="none"
            )
...
def get_renode_weight(data, data_train_mask, base_weight, max_weight):
    ...
    gpr_matrix = torch.tensor(
        data.gpr
    ).float()  # class-accumulated personlized pagerank
    ...
    rn_matrix[unlabel_mask] = (
        rn_matrix.max() + 99
    )  # exclude the influence of unlabeled node
    ...
    rn_weight = [
        (base_w + 0.5 * scale_w * (1 + math.cos(x * 1.0 * math.pi / (train_size - 1))))
        for x in totoro_rank
    ]
