...
def sampling_idx_individual_dst(class_num_list, idx_info, device):
    ...
    sampling_src_idx = [
        cls_idx[torch.randint(len(cls_idx), (int(samp_num.item()),))]
...
def saliency_mixup(
    ...
    if saliency != None:
        ...
        if dist_kl != None:  # Adaptive
            kl_mask = (torch.sigmoid(dist_kl / 3.0) * K).squeeze().long()
            idx_matrix = torch.arange(K).unsqueeze(dim=0).to(
                kl_mask.device
            ) >= kl_mask.unsqueeze(dim=1)
            zero_repeat_idx = mask_idx[:, 0:1].repeat(1, mask_idx.size(1))
            mask_idx[idx_matrix] = zero_repeat_idx[idx_matrix]

        lam[torch.arange(lam.shape[0]).unsqueeze(1), mask_idx] = 1.0
...
def duplicate_neighbor(total_node, edge_index, sampling_src_idx):
    ...
    new_row = (
        torch.arange(len(sampling_src_idx)).to(device) + total_node
    ).repeat_interleave(degree[sampling_src_idx])
    ...
    b_idx = (
        torch.arange(len(unique_src)).to(device).repeat_interleave(degree[unique_src])
    )
...
def get_dist_kl(prev_out, sampling_src_idx, sampling_dst_idx):
    ...
    dist_kl = F.kl_div(
        torch.log(prev_out[sampling_dst_idx.to(device)]),
        prev_out[sampling_src_idx.to(device)],
        reduction="none",
...
def neighbor_sampling(
    ...
    if prev_out is not None:
        ...
        ratio = F.softmax(
            torch.cat([dist_kl.new_zeros(dist_kl.size(0), 1), -dist_kl], dim=1), dim=1
        )
        mixed_neighbor_dist = ratio[:, :1] * neighbor_dist_list[sampling_src_idx]
        for i in range(n_candidate):
            mixed_neighbor_dist += (
                ratio[:, i + 1 : i + 2]
                * neighbor_dist_list[sampling_dst_idx.unsqueeze(dim=1)[:, i]]
            )
    else:
    ...
    degree_dist = (
        scatter_add(torch.ones_like(degree[train_node_mask]), degree[train_node_mask])
        .to(device)
        .type(torch.float32)
    )
